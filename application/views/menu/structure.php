<div class="container-fluid">

    <!-- start page title -->
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Organization Structure </h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="javascript: void(0);">Structure </a></li>
                        <li class="breadcrumb-item active">Index<i class="mdi mdi-alpha-x-circle:"></i></li>
                    </ol>
                </div>

            </div>
        </div>
    </div>

    <div class="col-xl-12">
        <h4 class="text-center my-4 fw-bold">CORPORATE IT-SYSDEV ORGANIZATION STRUCTURE</h4>
        <div class="container text-center">
            <!-- Corporate Audit Group -->
            <div class="row justify-content-center text-center">
                <div class="col-md-4">
                    <img src="http://172.16.161.34:8080/hrms/images/users/43864-2013=2020-02-14=Profile=16-16-40-PM.jpg" class="img-thumbnail avatar-md rounded-circle" style="border-color: orange">
                    <h6 class="mt-2 fw-bold fs-12">MARIA NELIZA U. FUERTES</h6>
                    <p class="text-muted fs-6">Corporate Audit Group Managing Directress</p>
                </div>
            </div>
            <!-- Corporate IT Manager -->
            <div class="row justify-content-center text-center mt-1">
                <div class="col-md-4">
                    <img src="http://172.16.161.34:8080/hrms/images/users/14484-2013=2023-09-04=Profile=10-28-55-AM.JPG"
                        class="img-thumbnail avatar-md rounded-circle" style="border-color: orange">
                    <h6 class="mt-2 fw-bold fs-12">HENRY D. BALIAR</h6>
                    <p class="text-muted fs-6">Corporate IT Manager</p>
                </div>
            </div>
            <!-- Corporate Accounting Manager -->
            <div class="row justify-content-center text-center mt-1">
                <div class="col-md-4">
                    <img src="http://172.16.161.34:8080/hrms/images/users/21114-2013=2024-01-25=Profile=16-54-41-PM.jpg"
                        class="img-thumbnail avatar-md rounded-circle" style="border-color: orange">
                    <h6 class="mt-2 fw-bold fs-12">MARIA CRISTINA C. EVARLE</h6>
                    <p class="text-muted fs-6">IT Sysdev - Jr. Supervisor</p>
                </div>
            </div>

            <!-- IT SysDev Section Heads -->
            <div class="row justify-content-center text-center mt-1">
                <div class="col-md-6">
                    <img src="http://172.16.161.34:8080/hrms/images/users/01022-2014=2024-01-25=Profile=16-51-55-PM.jpg"
                        class="img-thumbnail avatar-md rounded-circle" style="border-color: orange">
                    <h6 class="mt-2 fw-bold fs-12">JENEFFER C. CUTANDA</h6>
                    <p class="text-muted fs-6">IT SysDev Section Head</p>
                </div>
                <div class="col-md-6">
                    <img src="http://172.16.161.34:8080/hrms/images/users/06129-2013=2024-01-26=Profile=14-01-22-PM.jpg"
                        class="img-thumbnail avatar-md rounded-circle" style="border-color: orange">
                    <h6 class="mt-2 fw-bold fs-12">MARK ANTHONY S. RABACA</h6>
                    <p class="text-muted fs-6">IT SysDev Section Head</p>
                </div>
            </div>

            <div class="container text-center mt-5">
                <!-- Programmers Section -->
                <h5 class="fw-bold mb-4 mt-5 text-uppercase">Programmers</h5>
                <div class="row justify-content-center">
                    <?php foreach ($programmers as $pro) : ?>
                        <div class="col-md-2 mb-3">
                            <img src="http://172.16.161.34:8080/hrms/<?php echo $pro['photo']; ?>"
                                class="img-thumbnail avatar-md rounded-circle" style="border-color: orange">
                            <h6 class="mt-2 text-muted text-uppercase"><?php echo $pro['name']; ?></h6>
                        </div>
                    <?php endforeach; ?>
                </div>

                <!-- System Analyst Section -->
                <h5 class="fw-bold mb-4 mt-5 text-uppercase">System Analyst</h5>
                <div class="row justify-content-center">
                    <?php foreach ($analysts as $sa) : ?>
                        <div class="col-md-2 mb-3">
                            <img src="http://172.16.161.34:8080/hrms/<?php echo $sa['photo']; ?>"
                                class="img-thumbnail avatar-md rounded-circle" style="border-color: orange">
                            <h6 class="mt-2 text-muted text-uppercase"><?php echo $sa['name']; ?></h6>
                        </div>
                    <?php endforeach; ?>
                </div>

                <!-- RMS Teams Section -->
                <h5 class="fw-bold mb-4 mt-5 text-uppercase">RMS Teams</h5>
                <div class="row justify-content-center">
                    <?php foreach ($others as $r) : ?>
                        <div class="col-md-2 mb-3">
                            <img src="http://172.16.161.34:8080/hrms/<?php echo $r['photo']; ?>"
                                class="img-thumbnail avatar-md rounded-circle" style="border-color: orange">
                            <h6 class="mt-2 text-muted text-uppercase"><?php echo $r['name']; ?></h6>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>


        </div>

    </div>
</div>