            ...
            $data[] = [
                ...
                'action'        => '
                    <div class="justify-content-center">
                        <button type="button" class="btn btn-soft-secondary btn-label waves-effect waves-light btn-sm" onclick="upload_remaining_files(' . $row['mod_id'] . ')" data-bs-toggle="modal" data-bs-target="#upload_remaining_files">
        ...
        foreach ($remaining_directories as $row) {
            ...
            foreach ($filtered_directories as $directory) {
                $data[] = [
                    ...
                    'action' => '
                        <div class="justify-content-center">
                            <button type="button" class="btn btn-soft-secondary btn-label waves-effect waves-light btn-sm"  onclick="upload_to_directory(' . $mod_id . ', \'' . $directory . '\', \'' . $row['sub_mod_id'] . '\', \'' . $row['team_id'] . '\', \'' . $row['typeofsystem'] . '\')">
        ...
        if ($typeofsystem === 'new') {
            ...
            if ($manager_key) {
                ...
                if ($current_index > 0) {
                    ...
                    $pending_files = $this->file_mod_current->get_pending_files($team, $module, $sub_module, $previous_directory);
                    ...
                    $previous_files_exist = $this->file_mod_current->check_files_exist($team, $module, $sub_module, $previous_directory);
                    if (!$previous_files_exist) {
                        $response['message'] = "Please upload files to the \"$previous_directory\" directory before proceeding to \"$selected_directory\".";
                        echo json_encode($response);
                        return;
